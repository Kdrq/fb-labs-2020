...
file = open("E:/lab2/texti.txt", "r", encoding='utf-8')
checks = file.read()
checks = checks.lower()
checks = checks.replace('ё','е')
checks = re.sub('[^а-я]', '', checks)
open("proverka", 'w').write(checks)
text = open('proverka', 'r').read()
...
encode_f(text, 'да') #2
encode_f(text, 'нет') #3
encode_f(text, 'лаба') #4
encode_f(text, 'львов') #5
encode_f(text, 'следующими') #10
encode_f(text, 'машиностроение') #14

open('key2.txt', 'w').write(encode_f(text, 'да'))
open('key3.txt', 'w').write(encode_f(text, 'нет'))
open('key4.txt', 'w').write(encode_f(text, 'лаба'))
open('key5.txt', 'w').write(encode_f(text, 'львов'))
open('key10.txt', 'w').write(encode_f(text, 'следующими'))
open('key14.txt', 'w').write(encode_f(text, 'машиностроение'))
...
confirm_text = open("proverka", "r").read()

def indeks_count(some_dict, number):
    result_index=0
    for i in some_dict:
...
indeks_count(Counter(text), confirm_text)
print('theoretical value:', indeks_count(Counter(text), confirm_text))

def text_read(name):
    t = open(name, 'r').read()
    return t
print('ключ "да":',indeks_count(Counter(text_read('key2.txt')), confirm_text))
print('ключ "нет":',indeks_count(Counter(text_read('key3.txt')), confirm_text))
print('ключ "лаба":',indeks_count(Counter(text_read('key4.txt')), confirm_text))
print('ключ "львов":',indeks_count(Counter(text_read('key5.txt')), confirm_text))
print('ключ "следующими":',indeks_count(Counter(text_read('key10.txt')), confirm_text))
print('ключ "машиностроение":',indeks_count(Counter(text_read('key14.txt')), confirm_text))
...
encrypted=open("E:/lab2/vigenere.txt",'r',encoding='utf-8').read()
...
def f(step, some_encrypt_file):
    ...
    for j in range(0,step): 
        ...
        indeks_list.append(indeks_count(temp_string_freq, temp_string)) 
    return sum(indeks_list)/len(indeks_list)
        
        
d1=dict((f(r,encrypted), r) for r in range(1,33))
...
def f1(number_block,encrypted,r):
    temp_list=[]
    for i in range(number_block,len(encrypted),r):
        temp_list.append(encrypted[i])
...
decode_func(encrypted,key_of_text )
print(decode_func(encrypted,"экомаятникфуко"))